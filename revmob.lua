...
local d=nil
local c=nil
if t.getSessionStarted()then
s=system.getInfo("iosAdvertisingIdentifier")d=system.getInfo("iosIdentifierForVendor")c=system.getInfo("iosAdvertisingTrackingEnabled")end
local l=nil
local n=nil
if r~=nil then
...
end)package.preload['revmob_client']=(function(...)local p=require('json')local h=require('socket.http')local d=require("ltn12")local e=require('revmob_about')local n=require('revmob_log')local t=require('revmob_messages')local o=require('revmob_events')local u=require('revmob_context')local m=require('revmob_cache')local i=require('revmob_device')local s=require('revmob_utils')REVMOB_ID_IOS='iPhone OS'REVMOB_ID_ANDROID='Android'local e
local v=30
local g='https://ios.revmob.com'local c='https://android.revmob.com'local L="app"local D=false
local k=true
local E="Download a FREE game!"local A=10
local f=true
local r=0
local l=nil
local function a()if"Android"==system.getInfo("platformName")then
return c
else
return g
end
...
local function g(o,t,i,n)if n==nil then
local n key="fetch_"..t
local e=e.serverEndPoints[key]if e~=nil then
return e
end
return a().."/api/v4/mobile_apps/"..o.."/"..i.."/fetch.json"else
local t="fetch_"..t.."_with_placement"local e=e.serverEndPoints[t]if e~=nil then
e=string.gsub(e,"PLACEMENT_ID",n)return e
end
return a().."/api/v4/mobile_apps/"..o.."/placements/"..n.."/"..i.."/fetch.json"end
end
local function I(n)local e=e.serverEndPoints['install']if e~=nil then
return e
else
return a().."/api/v4/mobile_apps/"..n.."/install.json"end
end
local function _(e)return a().."/api/v4/mobile_apps/"..e.."/sessions.json"end
...
local function b(n)if n==nil then return nil end
local e
...
if(t==200)then
m.saveInstallWasRegistered(e.appId)n.release("Install received.")if listener~=nil then
...
c(I(e.appId),n)end
local function a(r,a,i,l)local i=b(i)if e.sessionStarted then
if i~=nil then
n.info("Ad registered with Placement ID "..i)end
c(g(e.appId,r,a,i),l)else
...
local u
local function g(n)local t={}local e=""local i,e,t=h.request{method="POST",url=n,source=d.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=d.sink.table(t),}return u(n,e,t)end
...
e={TEST_WITH_ADS="with_ads",TEST_WITHOUT_ADS="without_ads",TEST_DISABLED=nil,appId=nil,appIds=nil,sessionStarted=false,testMode=nil,listenersRegistered=false,serverEndPoints={},ipAddress=nil,startSession=function(r,l)local t=b(r)if w(t)then
...
end,registerInstall=function()if m.isInstallRegistered(e.appId)then
n.info("Install already registered in this device")else
S()end
end,fetchFullscreen=function(n,e)a('fullscreen','fullscreens',n,e)end,fetchBanner=function(n,e)a('banner','banners',n,e)end,fetchLink=function(e,n)a('link','anchors',e,n)end,fetchPopup=function(e,n)a('pop_up','pop_ups',e,n)end,reportImpression=function(e)if e~=nil then
...
e=""end
local i,e,t=h.request{method="POST",url=n,source=d.source.string(e),headers={["Content-Length"]=tostring(#e),["Content-Type"]="application/json"},sink=d.sink.table(t),}return u(n,e,t)end,getMarketURLWithoutFollow=function(e)f=false
local e=g(e)f=true
return e
...
e.setTimeoutInSeconds(v)return e
end)package.preload['revmob_fullscreen_web']=(function(...)local r=require('revmob_log')local a=require('revmob_messages')local l=require('revmob_events')local n=require('revmob_client')require('revmob_sound_player')local i="fullscreen"local o
o={autoshow=true,listener=nil,impressionUrl=nil,clickUrl=nil,htmlUrl=nil,dspHtml=nil,campaignId=nil,new=function(e)local e=e or{}setmetatable(e,o)return e
end,load=function(e,o)e.networkListener=function(t)local t,o=n.theFetchSucceed(i,t,e.listener)if t then
local t=o['fullscreen']['links']e.impressionUrl=n.getLink('impressions',t)e.clickUrl=n.getLink('clicks',t)e.htmlUrl=n.getLink('html',t)r.release("Fullscreen loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=l.AD_RECEIVED,ad=i})end
if e.autoshow then
e:show()end
end
end
n.fetchFullscreen(o,e.networkListener)end,isLoaded=function(e)return(e.htmlUrl~=nil and e.clickUrl~=nil)or(e.dspHtml~=nil)end,hide=function(e)if not e:isLoaded()then e.autoshow=false end
...
e.clickListener=function(o)if string.sub(o.url,-string.len("#close"))=="#close"then
...
if string.sub(o.url,-string.len("#click"))=="#click"then
if e.changeOrientationListener then
Runtime:removeEventListener("orientation",e.changeOrientationListener)end
...
if o.errorCode then
r.release("Error: "..tostring(o.errorMessage))end
return true
end
...
Runtime:removeEventListener("orientation",e.changeOrientationListener)end
native.cancelWebPopup()end,playSoundOnShow=function(e)SoundPlayer.playSoundFromURL(e.showSoundURL)end}o.__index=o
return o
end)package.preload['revmob_fullscreen_static']=(function(...)local t=require('revmob_log')local a=require('revmob_messages')local o=require('revmob_events')local r=require('revmob_utils')local s=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local i="fullscreen"local l
l={autoshow=true,listener=nil,impressionUrl=nil,clickUrl=nil,imageUrl=nil,closeButtonUrl=nil,component=nil,campaignId=nil,_clicked=false,_released=false,_updateAccordingToOrientation=nil,_loadCloseButtonListener=nil,_loadImageListener=nil,_networkListener=nil,_moveToFront=nil,new=function(e)local e=e or{}setmetatable(e,l)e.component=display.newGroup()e.component.alpha=0
...
r.loadAsset(e.imageUrl,e._loadImageListener,"fullscreen.jpg")end,loadCloseButton=function(e)if e._released==true then return end
...
r.loadAsset(e.closeButtonUrl,e._loadCloseButtonListener,"close_button.jpg")end,_updateResourcesLoaded=function(e)if e:isLoaded()then
...
e.image.width=r.Screen.width()e.image.height=r.Screen.height()end
...
e._clicked=false
t.info("Fullscreen closed")end,playSoundOnShow=function(e)SoundPlayer.playSoundFromURL(e.showSoundURL)end}l.__index=l
return l
...
return i
end)package.preload['revmob_banner_static']=(function(...)local i=require('revmob_log')local s=require('revmob_messages')local r=require('revmob_events')local t=require('revmob_utils')local a=require('revmob_device')local n=require('revmob_client')require('revmob_sound_player')local o="banner"local l
l={autoshow=true,listener=nil,impressionUrl=nil,clickUrl=nil,campaignId=nil,imageUrl=nil,component=nil,_clicked=false,_released=false,shouldRelease=true,width=nil,height=nil,x=nil,y=nil,rotation=0,new=function(e)local e=e or{}setmetatable(e,l)e.component=display.newGroup()e.component.alpha=0
return e
...
e._loadImageListener=function(l)if e._released==true then if l.target then l.target:removeSelf()end i.info("Banner was released.")return end
if l.isError or l.target==nil or e.imageUrl==nil then
i.release("Fail to load ad image: "..tostring(e.imageUrl))if e.listener~=nil then e.listener({type=r.AD_NOT_RECEIVED,ad=o})end
return
end
e.image=l.target
local l=(t.Screen.width()>640)and 640 or t.Screen.width()local a=(a.isIPad()and 100 or 50*(t.Screen.bottom()-t.Screen.top())/display.contentHeight)local c=(t.Screen.left()+l/2)local d=(t.Screen.bottom()-a/2)e:setPosition(e.x or c,e.y or d)e:setDimension(e.width or l,e.height or a)e.image.tap=function(t)if not e._clicked then
e._clicked=true
if e.listener~=nil then
e.shouldRelease=false
e.listener({type=r.AD_CLICKED,ad=o})end
local t=nil
if e.followRedirect==true then
t=n.getMarketURL(e.clickUrl)else
t=n.getMarketURLWithoutFollow(e.clickUrl)end
...
e.image.touch=function(n)return true end
e.image:addEventListener("tap",e.image)e.image:addEventListener("touch",e.image)e.component:insert(1,e.image)i.release("Banner loaded - "..e.campaignId)if e.listener~=nil then e.listener({type=r.AD_RECEIVED,ad=o})end
if e.autoshow then
e:show()end
end
...
if e.component~=nil then
e.component.alpha=1
if e.listener~=nil then e.listener({type=r.AD_DISPLAYED,ad=o})end
e:playSoundOnShow()n.reportImpression(e.impressionUrl)end
...
function string:split(i,e)if not e then
e={}end
local n=1
local t,o=string.find(self,i,n)while t do
table.insert(e,string.sub(self,n,t-1))n=o+1
t,o=string.find(self,i,n)end
table.insert(e,string.sub(self,n))return e
end
...
end)package.preload['revmob_popup']=(function(...)local l=require('revmob_log')local a=require('revmob_messages')local o=require('revmob_events')local t=require('revmob_client')require('revmob_sound_player')local d=require("socket.url")local i="popup"local s=2
...
e.message=t
e.campaignId=""local n=d.parse(e.clickUrl)local n=n["query"]if n~=nil then
local n=n:split("&")for t=1,#n do
if string.find(n[t],"campaign_id=")~=nil then
local n=n[t]:split("=")e.campaignId=n[2]end
end
...
end,parameters=function()DownloadManager.params.progress="download"DownloadManager.params.response={filename=DownloadManager.getFileName(),baseDirectory=system.DocumentsDirectory}end,download=function(e,n)DownloadManager.setFileName(n)DownloadManager.parameters()network.request(e,"GET",DownloadManager.networkListener,DownloadManager.params)end}return DownloadManager end)local i=require('revmob_log')local e=require('revmob_context')local n=require('revmob_client')require('revmob_fullscreen')require('revmob_banner')require('revmob_link')require('revmob_popup')local t
t={TEST_DISABLED=n.TEST_DISABLED,TEST_WITH_ADS=n.TEST_WITH_ADS,TEST_WITHOUT_ADS=n.TEST_WITHOUT_ADS,startSession=function(e)local e=n.startSession(e,nil)end,startSessionWithListener=function(t,e)local e=n.startSession(t,e)end,setTestingMode=function(e)n.setTestingMode(e)end,showFullscreen=function(e,n)local e=t.createFullscreen(e,n)e:show()return e
